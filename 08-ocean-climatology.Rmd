...
Here we describe the creation of a continuous, monthly-resolution ocean climatology for the Perth coastal waters region, focusing on temperature, salinity, and biogeochemical properties which are needed to inform boundary conditions for the CSIEM platform and associated analyses. We expand the concept of regional ocean climatologies by incorporating biogeochemical data alongside predictions from the available regional hydrodynamic model. To achieve this, we undertake a synthesis by combining the regional resolution ROMS model, satellite imagery, and available *in-situ* data. The resulting data product covers the period from 1980 to 2024, and provides continuous key ocean water properties that can be used for boundary condition specification for our localised models of Perth's coastal waters (which are described further in Chapter 9 and 11).
...
Standardising historical observations poses a significant challenge in climatological studies, particularly regarding metadata consistency and quality control. The process of data collection and integration into a standardised database is detailed in the WWMSP Theme 1.1 report -- Integrated Ecosystem Modelling Platform. For this study, ocean climatology analysis utilises observations from various sources, including:
...
It is also necessary to understand the effect of inter-annual variability in sea level drivers, that shape what is being experienced within Cockburn Sound. Figure \@ref(fig:sea-level-anomaly) demonstrates that the mean water level in any given year can be explained based on i) a slow rate of sea level rise and ii) an annual anomaly linked with the ENSO index (as captured here using the R-Nino3.4 metric). The selection of assessment years can factor in this variability when identifying CSIEM validation data-sets to ensure a range of conditions are represented.   
...
*Note on bias-corrected boundary condition files* : This chapter provides a general summary of the rationale and approach for bias correction. Please refer to further analysis in [Chapter 10.8](salt-boundary) summarising the model validation using this approach.
...
This analysis yielded a CHL:POC ratio of 0.0044, which is aligned with the typical CHL:POC ratio of 0.004-0.010 in Southeast Indian Ocean (Strutton *et al.*, 2023). The POC concentrations at the nearshore area were then estimated from the TCHLA concentration using the CHL:POC ratio of 0.0044.
...
We subsequently estimated the concentrations of particulate and dissolved organic nitrogen and phosphorus (PON, POP, DON, DOP) from the POC and DOC concentrations using C:N:P stoichiometric ratios. The Redfield ratio (106:16:1) was applied as a baseline, assuming phyto-detritus dominates the organic matter.
...
Based on this information, we further categorised the phytoplankton into four functional types, which are:
...
Zooplankton biomass tends to follow the patterns observed with phytoplankton abundance (Yuan and Pollard, 2018), and this is the case in Cockburn Sound (Figure \@ref(fig:zoo-final)). It is therefore reasonable to set up a zooplankton biomass associated with the phytoplankton biomass when specifying boundary zooplankton biomass amounts.
...
The main aim of this analysis was to create a monthly resolution reference climatology of salinity, temperature, nutrient species and other biogeochemical variables that could be used for local scale models. We have designed this to span from early time-periods in CS history until present in order to enable long-term simulations within the CSIEM platform. The systematic screening and analysis of the available datasets, as shown in section 6.3.1 -- 6.3.4, has help to optimise the ocean boundary conditions (OBCs) for CSIEM, that are summarized in Table \@ref(tab:obc-summary). With the monthly climatology approach, CSIEM v1.5+ is able to utilise continuous ocean conditions in space and time when and where data is not available. The final OBC products showed a clear seasonal pattern in most of the properties. For example, during the summertime, the salinity and temperature are generally higher, whilst the nutrient and phytoplankton biomass are generally lower. In contrast, salinity and temperature are lower in wintertime with elevated nutrient concentrations and phytoplankton biomass.
...
b.  Typical Chl-a/POC ratio is 0.004-0.010 in Southeast Indian Ocean (Strutton *et al.*, 2023); Chl:POC ratio =0.0044 in the NRSROT IMOS station <br>
