...
The site locations of these data sources are shown in Figure 6.1.
...
**Figure 6.1.** Map of boundary polygons, and stations where observed data was sourced. *Note the ROMS water levels, currents, temperatures and salinities within CSIEM are applied continuously along the ocean-edge of the hydrodynamic model, and the six polygons referred to here specifically relate to: i) bias-correction factors, and ii) regions for assignment of biogeochemical properties*.
...
Comparison of ROMS output of surface elevation with the Fremantle Fishing Boat Harbour tidal gauge station indicates that the field observations compare well with ROMS prediction (Figure 6.2), including ability to capture the temporal variations in tidal elevations throughout the year ($R=0.9391$). The minimum bias between the model and observation was with a datum offset of -0.34 meters, which is thus applied to the tidal elevations from the ROMS outputs. The comparison should be viewed with caution noting that the Fremantle gauge is within a Harbour and will experience more complex controls on tidal highs and lows which are not expected to be reflected within the offshore boundary inputs to CSIEM. 


![](./media/image12.png){width="6.301388888888889in" height="2.5479166666666666in"}

Figure **6.2.** (left) Time series and (right) regression analysis of 2km WA-ROMS surface elevation against the observations at Fremantle gauge station.


It is also necessary to understand the effect of inter-annual variability in sea level drivers, that shape what is being experienced within Cockburn Sound. Figure 6.3 demonstrates that the mean water level in any given year can be explain based on i) a slow rate of sea level rise and ii) an annual anomoly linked with the ENSO index (as captured here using the R-Nino3.4 metric). The selection of assessment years can factor in this variability when identifying CSIEM validation data-sets to ensure a range of conditions are represented.   

![](./media/climatology/fremantle_vs_rnino_v2.png){width=70%}

Figure **6.3.** Long-term drivers of the sea-level anomaly as recorded at the Fremantle Fishing Boat Harbour.

### Boundary climatology -- *water temperature and salinity*  {#boundary-salinity}

The ROMS model outputs from WWMSP Project 5.1 for the WA region were used to provide hydrographic boundary conditions for CSIEM (for details of model setup refer to Pattiaratchi and Janekovic, 2024). The outputs include tidal elevation, water temperature, salinity, and ocean currents at three-hourly intervals. Example snapshots of ROMS surface water temperature in comparison to the [GHRSST products](https://www.ghrsst.org) (Figure 6.3) showed that the ROMS and GHRSST data-sets present similar spatial temperature patterns with higher temperatures concentrated in the northern region that decreased towards the south. The ROMS model exhibits slightly more uniform temperature gradients compared to GHRSST, which displays more localized high-temperature zones, particularly closer to the coastal areas. Similar plots comparing the ROMS salinity and the MOI global simulation (Figure 6.5) suggest that the ROMS and MOI salinity distributions agree on broader spatial salinity trends, with a slight increase of salinity from north to south. However, the ROMS model shows slight differences in the Perth region, which are explored further next.

![](./media/image8.png){width=90%}

**Figure 6.4.** Snapshots of surface water temperature from ROMS (left column) and GHRSST satellite data (right column).

![](./media/image9.png){width=90%}

**Figure 6.5.** Snapshots of surface salinity from ROMS (left column) and MOI (right column).

A further benchmarking analysis of ROMS outputs against observational data from IMOS (Rottnest Island station), DWER (at site s6142985), and CSIRO (near Two Rocks) indicated that the ROMS effectively captured the water temperature, with correlation coefficients (*r*) greater than 0.86 and bias less than 2% at all sites (Figure 6.6). However, ROMS exhibited a tendency to underpredict salinity during the summers and overpredict it during the winters, particularly at nearshore site where salinity can exceed 37 PSU in summers and drop below 35 PSU in winters.
...
The bias factors derived from the IMOS station were applied to offshore polygons 3 and 4, while the factors from the DWER nearshore station were applied to nearshore polygons 1 and 6. For the transitional polygons 2 and 5, the average bias factors from both the IMOS station and DWER stations were applied. Figure 6.5 presents the time series plots of the ROMS data, field observational data, and the bias-corrected ROMS data. The regressions of the bias-corrected salinity against the field observations are presented in Figure 6.4, demonstrating that the bias correction method has significantly improved the accuracy of salinity predictions in the ROMS model.

![](./media/image10.png){width="6.294344925634296in" height="4.49369750656168in"}

**Figure 6.6.** Regression analysis of ROMS salinity (SAL) and water temperature (TEMP) against the observations at IMOS, DWER s6142985, and the CSIRO A stations. Regression coefficients (r) and bias between the ROMS outputs and observations are included in the plots. For SAL, the light blue colour represents results from the raw ROMS output vs. data and the orange colour represents results from the bias-correction method.

![](./media/image11.png){width="6.301388888888889in" height="4.447553587051619in"}

**Figure 6.7.** Multi-year time-series comparing the WA-ROMS salinity and temperature (blue dots) against the observations from IMOS, DWER, and CSIRO data-sets (green dots). The orange lines indicate the bias-corrected salinity predictions from the ROMS model. Click to enlarge.


```{block2, infoM6-1, type='rmdnote2'}

*Note on bias-corrected boundary condition files* : This chapter provides a general summary of the rationale and approach for bias correction. Please refer to further analysis in Section 7.X summarising the model validation using this approach.
...
The time series comparison (Figure 6.7 and 3.8) indicates that while the global ocean model for FRP, NIT, and DO generally align in magnitudes with field observations at the IMOS and DWER sites, they lack the strong seasonal variation observed in field data, and the regression results show generally poor agreement. Consequently, we relied solely on the field observations to construct the monthly climatology for the nutrient conditions at the boundaries. The monthly average values of dissolved nitrogen, phosphorus and DO, $C_{i}$, is calculated as follows:
