...
**Figure 13.1.** Snapshots taken from Sentinel-2 imagery showing periods where different optical properties can dominate water properties in and around Cockburn Sound.
...
A range of high quality light-related data-sets have been collected for assessing the light, including long-term monitoring of light attenuation, above and below water sensors measureing *in situ* $PAR$, and several spectral (multi-wavelength) light sensor deployments. Refer to Figure 13.2) for a broad summary of past data collection efforts, and Table \@ref(tab:13-lightdata) for a summary of key data sets considered during model assessment. 

![](./media/wq/CS_sites_light_2.png){width="90%"}

**Figure 13.1.** Light data-set: Summary of CSIEM data catalogue showing identified hydrodynamics data. Click to enlarge. Visit the csiem-data catalogue for further information.  
<br>
...
kbl(theSheet[,3:7], caption = "Summary of key data-sets included within CSIEM. Refer to Table 12.X for data sets reporting TSS and turbidity.", align = "l",) %>%
...
Photosynthetically Active Radiation (PAR; 400–700 nm) decreases with depth as light is absorbed and scattered by pure water, suspended inorganic sediments, phytoplankton pigments, and particulate and
...
where \(I_{\text{PAR}}(0^-)\) is PAR just below the water surface. Within AED, this calculation is resolved discretley over the vertical layer increments, such that the light at the bottom of each vertical layer is  calculated from the bulk layer properties:
...
Whilst the model is simple and applied widely, the challenge is locally parameterising the specific-attenuation coefficients, $k_{\text{e}...}$, in order to accurately reflect local conditions.  For Cockburn Sound, we reviewed literature values for similar embayments and local estimates, and then undertook refinement by a) adjusting of the specific attenuation factors to match $K_d$ observations (specifically for $SS$), and b) validating light attenuation against benthic PAR sensors (see 13.2 for a description of the available data-sets). A summary table of coefficients in shown in Table 12.1.

**Table 13.2.** PAR attenuation parameters

| Variable | Units | Default CSIEM value | Typical values used within coastal embayments |
|---------|--------|------------------------|----------------------------------------------------|
| \(K_{w}\) | $m^{-1}$ | 0.05 | Background water attenuation typically 0.04–0.07 in clear coastal waters |
| \(k_{\text{e_ss1}}\) | $m^{-1}/ (g/m^{-3})^{-1}$ | 0.085  | Fine suspended sediments: 0.01–0.03; higher attenuation rates for silts and clays with strong scattering properties. |
| \(k_{\text{ss2}}\) | $m^{-1}/ (g/m^{-3})^{-1}$ |  0.045 | Coarser inorganic particles (fine sands): $0.002–0.01$; attenuation drops with increasing particle size as less scattering per unit mass (Bowers & Binding 2006). Default value of $0.05$ commonly adopted.  |
| \(k_{\text{poc}}\) | $m^{-1}/ (mmol\:C/m^{-3})^{-1}$| 0.00096  | POC optical effects vary by composition; equivalent to $0.001–0.01 \: m^2 mg C^{-1}. For mmol C units, this corresponds approximately to $0.012–0.12\: m^2 mmol C^{-1} (using 1 mmol C ≈ 12 mg C). |
| \(k_{\text{doc}}\) | $m^{-1}/ (mmol\:C/m^{-3})^{-1}$ | 0.0005  | DOC influences light mainly via CDOM absorption. Typical coastal CDOM attenuation 0.001–0.02 m\(^2\) mg C\(^{-1}\), equivalent to 0.012–0.24 m\(^2\) mmol C\(^{-1}\). This term is usually elevated near riverine or groundwater inputs with high terrestrial DOM inputs. |
| \(k_{\text{phy}}\) | $m^{-1}/ (mmol\:C/m^{-3})^{-1}$ | 0.0030 - 0.0051  | Phytoplankton chlorophyll-specific attenuation 0.015–0.03 m\(^2\) mg\(^{-1}\); ~0.02 m\(^2\) mg\(^{-1}\) commonly used in estuarine models. This term influences extinction due to algal blooms. O2M (2025) estimated phytoplankton attenuation at $0.256\: /m$, which for a concentration of $2 ug/L$ of chlorophyll-a converts to $~0.03 m^{-1}/ (mmol\:C/m^{-3})^{-1}$ |


### $K_d$ validation

Since the simulated $K_d$ can have variable drivers, due to the numerous factors outlined in Table 13.2, the predictions are validated against local observations. The light attenuation coefficient has been measured across three main data collection programs (Table \@ref(tab:13-lightdata)). For brevity, the model is compared against the four broad assessment regions which each contain several regular data sampling locations for the year of 2023 (Figure 13.3). For comparison against the CSMC data or local sampling sites, over different years refer to the MARVL viewer. 
...
![](./media/wq/EXTC_2023_combined/region/0014_CS_Region.png){width=90%}
...
![](./media/wq/EXTC_2023_combined/region/0023_OA_Region.png){width=90%}
...
![](./media/wq/EXTC_2023_combined/region/0036_GR_Region.png){width=90%}

**Figure 13.3-iii.** Comparison of observed and simulated light extinction coefficient ($K_d,\:  /m$), within Gage Roads. 

##### South Coast {.unnumbered}

![](./media/wq/EXTC_2023_combined/region/0059_SW_Region.png){width=90%}

**Figure 13.3-iv.** Comparison of observed and simulated light extinction coefficient ($K_d,\: /m$), within the South Coast region. 


### Benthic $PAR$ validation

The bulk-PAR light model within AED was assessed using the WWMSP2-MS9 data-set (REF et al., 2024). The data-set sits at a sebed depth of ~8 m on the western edge of Kwinana Shelf, and shows a range of conditions spanning from summer to 2022/2023 through to winter of 2023. Figure 13.XX below shows six 2-3 week periods where conditions were assessed. For this comparison the data was assumed to sit 20 cm above the seabed, and this was compared with the upper face of the bottom-most model layer.
...
![](./media/wq/PAR_KS_combined/20221128_20221214_KwinanaShelf.png){width=90%}
...
![](./media/wq/PAR_KS_combined/20230102_20230118_KwinanaShelf.png){width=90%}
...
![](./media/wq/PAR_KS_combined/20230216_20230304_KwinanaShelf.png){width=90%}
...
![](./media/wq/PAR_KS_combined/20230318_20230403_KwinanaShelf.png){width=90%}
...
![](./media/wq/PAR_KS_combined/20230502_20230518_KwinanaShelf.png){width=90%}

**Figure 13.4-v.** Comparison of observed and simulated light ($PAR,\:W/m^2$) on the Kwinana Shelf in May 2023. 

##### May (late){.unnumbered}

![](./media/wq/PAR_KS_combined/20230518_20230603_KwinanaShelf.png){width=90%}

**Figure 13.4-vi.** Comparison of observed and simulated light ($PAR,\:W/m^2$) on the Kwinana Shelf in May 2023. 
...
**Table 14.1.** Location of sites for model validation, including seafloor depth (metres below mean sea level).
...
**Figure 13.1**. Absorption and scattering (/m) of optically active constituent species at 400nm for each site over the sunlit portion of the two days of the CSIEM simulation.
...
![](./media/image75.png){width="6.268055555555556in" height="7.834722222222222in"}**Figure 13.2**. Absorption and scattering (/m) of optically active constituent species at 550nm for each site over the sunlit portion of the two days of the CSIEM simulation.

![](./media/image76.png){width="6.268055555555556in" height="7.834722222222222in"}**Figure 13.3**. Absorption and scattering (/m) of optically active constituent species at 700nm for each site over the sunlit portion of the two days of the CSIEM simulation.
...
**Figure 13.6**. PAR at depth comparisons for sites Kwinana_Shelf, Mullaloo_Beach and Validation.

![](./media/image80.png){width="6.268055555555556in" height="7.521527777777778in"}**Figure 13.7**. PAR at depth comparisons for sites East_Garden Island, Freshwater_Bay and Mangles_Bay.

![](./media/image81.png){width="6.268055555555556in" height="7.521527777777778in"}**Figure 13.8**. PAR at depth comparisons for sites Deep_Basin and Owen_Anchorage.

![](./media/image82.png){width="6.268055555555556in" height="7.521527777777778in"}**Figure 13.9**. PAR at depth comparisons for site West_Rottnest.
...
![](./media/image83.png){width="6.268055555555556in" height="6.268055555555556in"}**Figure 13.10**. Daily energy flux in PAR band derived from EcoLight and CSIEM results for all nine sites at several depths.
...
![](./media/image84.png){width="6.268055555555556in" height="7.521527777777778in"}**Figure 13.11**. January 5, 2023 at noon (clear sky), spectral irradiances from EcoLight and CSIEM interpolated to 1m and 5m depths for the nine comparison sites.

![](./media/image85.png){width="6.268055555555556in" height="7.521527777777778in"}**Figure 13.12**. January 6, 2023 at noon (cloudy conditions), spectral irradiances from EcoLight and CSIEM interpolated to 1m and 5m depths for the nine comparison sites.

![](./media/image86.png){width="6.268055555555556in" height="7.521527777777778in"}**Figure 13.13**. January 5, 2023 at 4pm (clear sky), spectral irradiances from EcoLight and CSIEM interpolated to 1m and 5m depths for the nine comparison sites.

![](./media/image87.png){width="6.268055555555556in" height="7.521527777777778in"}**Figure 13.14**. January 6, 2023 at 4pm (cloudy conditions), spectral irradiances from EcoLight and CSIEM interpolated to 1m and 5m depths for the nine comparison sites.
...
Table 14.2. Dates, chlorophyll and TSS concentrations for the four in-situ comparison data sets.
...
We ran Hydrolight for the four scenarios outlined in Table 14.3. In the absence of a complete description of the optical conditions, we undertook model runs across a range of conditions to help identify the impact of this uncertainty.
...
![](./media/image90.png){width="6.263888888888889in" height="8.706421697287839in"} **Figure 14.16a.** Wavelength specific light profiles, showing best fit curves for light attenuation.
