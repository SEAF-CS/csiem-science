...
|---------|--------|------------------------|----------------------------------------------------|
| \(K_{w}\) | m\(^{-1}\) | 0.05 | Background water attenuation typically **0.04–0.07 m\(^{-1}\)** in clear coastal waters |
| \(k_{\text{e_ss1}}\) | m\(^2\) mg\(^{-1}\) | 0.085  | Fine suspended sediments: **0.01–0.03 m\(^2\) mg\(^{-1}\)**; higher attenuation rates for silts and clays with strong scattering properties. |
| \(k_{\text{ss2}}\) | m\(^2\) mg\(^{-1}\) |  0.045 | Coarser inorganic particles (fine sands): **0.002–0.01 m\(^2\) mg\(^{-1}\)**; attenuation drops with increasing particle size as less scattering per unit mass. |
| \(k_{\text{poc}}\) | m\(^2\) mmol C\(^{-1}\) | 0.00096  | POC optical effects vary by composition; equivalent to **0.001–0.01 m\(^2\) mg C\(^{-1}\)**. For mmol C units, this corresponds approximately to **0.012–0.12 m\(^2\) mmol C\(^{-1}\)** (using 1 mmol C ≈ 12 mg C). |
| \(k_{\text{doc}}\) | m\(^2\) mmol C\(^{-1}\) | 0.0005  | DOC influences light mainly via CDOM absorption. Typical coastal CDOM attenuation **0.001–0.02 m\(^2\) mg C\(^{-1}\)**, equivalent to **0.012–0.24 m\(^2\) mmol C\(^{-1}\)**. Strongly elevated values occur near riverine or groundwater inputs. |
| \(k_{\text{phy}}\) | m\(^2\) mmol C  \(^{-1}\) | 0.003 - 0.0051  | Phytoplankton chlorophyll-specific attenuation **0.015–0.03 m\(^2\) mg\(^{-1}\)**; ~0.02 m\(^2\) mg\(^{-1}\) commonly used in estuarine models. |


### $K_d$ validation

The light attenuation coefficient has been measured across three main programs as outlied in Table \@ref(tab:13-lightdata). Several of the assessment regions with with regular data from WWMSP sampling is showing in Figure 13.3 for the year of 2023. 
