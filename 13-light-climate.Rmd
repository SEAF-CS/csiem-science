...
Using the well-established Beer-Lambert equation, the downwelling PAR irradiance at depth \(z\) (m), denoted \(I_{\text{PAR}}(z)\)  (mol photons m\(^{-2}\) s\(^{-1}\)), is computed as:
...
where \(I_{\text{PAR}}(0^-)\) is PAR just below the water surface. Within AED, this continuous calculation is adjusted to compute the light at the bottom of each vertical layer, such that:
