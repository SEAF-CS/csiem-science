...
The condition of seagrass communities in Cockburn Sound, Warnbro Sound and Marmion at various sites were monitored annually between 2003 -- 2022 by Cockburn Sound Management Council (Figure 16.1). Sampling was mostly conducted during summer months but has varied from year to year. Survey records generally include shoot count, lower depth limit, seagrass height, dominant epiphyte type and cover (Table 16.1), but also varied over time. Monitoring primarily focused on the historically dominant species *Posidonia sinuosa*, though in some years, presence/absence or shoot count data are also available for other species such as *Posidonia australis*, *Posidonia coriacea*, *Amphibolis griffithii*, and *Amphibolis antarctica*. Average shoot count of *P. sinuosa* between 2003 - 2022 at all monitoring sites within the model domain are summarised in Figure 16.2 and 16.3. Additionally, several case studies that recorded *P. sinuosa* biomass in CS are particularly relevant to the dynamic seagrass model and are listed in Table 16.1.

```{r, echo=FALSE, out.width="93%"}
knitr::include_graphics("./media/image98.png")
```

:::{.figcap}
**Figure 16.1**. Location of CSMC long-term seagrass monitoring sites within the model domain (left) and within Cockburn Sound (right).
:::

**Table 16.1.** Summary of historical seagrass monitoring datasets in Cockburn Sound relevant to model development.
...
##### Sites (A--G) {.unnumbered}

```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics("./media/image99.png")
```

:::{.figcap}
**Figure 16.2-i.** Average *Posidonia sinuosa* shoot count per 20 cm^2^ between 2003 -- 2022 at CSMC monitoring sites (Becher Point -- Garden Island Settlement).
:::

##### Sites (H--N) {.unnumbered}

```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics("./media/image100.png")
```

:::{.figcap}
**Figure 16.2-ii.** Average *Posidonia sinuosa* shoot count per 20 cm^2^ between 2003 -- 2022 at CSMC monitoring sites (Hillarys Channel -- North Beach).
:::

##### Sites (O--W) {.unnumbered}

```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics("./media/image101.png")
```

:::{.figcap}
**Figure 16.2-iii.** Average *Posidonia sinuosa* shoot count per 20 cm^2^ between 2003 -- 2022 at CSMC monitoring sites (Ocean Reef Outer -- Wreck Rock).
:::

#### {.panelset .unnumbered}

##### Garden Island {.unnumbered}

```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics("./media/image102.png")
```

:::{.figcap}
**Figure 16.3-i.** Average *Posidonia sinuosa* shoot count per 20 cm^2^ between 2003 -- 2022 at Garden Island at multiple depths.
:::

##### Warnbro Sound {.unnumbered}

```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics("./media/image103.png")
```

:::{.figcap}
**Figure 16.3-ii.** Average *Posidonia sinuosa* shoot count per 20 cm^2^ between 2003 -- 2022 at Warnbro Sound at multiple depths.
:::

### Benthic habitat mapping

Map of the dominant benthic habitat features on the seafloor, with seagrass distribution shown to a genus level for *Amphibolis* and *Halophila* and species level for *Posidonia* for the Cockburn Sound, Owen Anchorage and Gage Roads regions were supplied in two versions. An earlier version was supplied by WAMSI Westport Marine Science Program Theme 2 Project 2.1 (WWMSP2.1), and a second version by O2M-Westport which included a few additional categories such as filter feeders. Benthic features for areas within the model domain not covered by this recent mapping were based on West Coast Bioregion (WCB) mapping (Hovey *et al.*, 2025).

Due to the high resolution of the supplied benthic habitat maps and technical constraints of TUFLOW-FV, polygon boundaries underwent a resolution reduction process prior to being used in model simulation (Figure 16.4). Additionally, the original seagrass community groups provided by WWMSP2.1 were classified into four groups by their dominant species (Table 16.2) to assist with alignment of group parameterisation within CSIEM. Both benthic map versions (WWMSP2.1 and WWMSP2.1-O2M-Westport merged) are compatible with CSIEM and can be selected during configuration. 


**Table 16.2**. Seagrass groups in WWMSP2.1 benthic map and CSIEM set up.
...
knitr::include_graphics("./media/image104.png")
```

:::{.figcap}
**Figure 16.4-i**. Substrate type and benthic habitat within the model domain based on WWMSP2.1 mapping, including distribution of seagrass communities within the Cockburn Sound and Owen Anchorage (CSOA). Click to enlarge.
:::

#### WWMSP2.1-O2M-Westport merged {.unnumbered}

```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics("./media/image133.png")
```

:::{.figcap}
**Figure 16.4-ii**. Substrate type and benthic habitat within the model domain based on merged WWMSP2.1-O2M-Westport mapping, including distribution of seagrass communities within the Cockburn Sound and Owen Anchorage (CSOA). Click to enlarge.
:::

### Seagrass experimental research

*Light threshold* : There have been several studies on the environmental thresholds of seagrass species that occur in CS over the last few decades, including field observations, field experiments and laboratory experiments. Among known environmental controls, light requirements received the most attention, with a number of metrics commonly seen in the literature such as minimum light requirement in the unit of percentage of surface irradiance (%SI) and daily light integral (DLI, mol m^-2^ d^-1^) over a set duration (see Said *et al.* 2024a for a comprehensive review). However, direct comparison between these metrics may be challenging (Said *et al.* 2024a). Some species have also been studied in a different geographical context (e.g. South Australia, Erftemeijer *et al.* 2023), which suggest that thresholds may differ between locations and locally derived thresholds are preferred.
...
The above equations are described in detail in [Chapter 15 of the AED Science Manual](https://aquaticecodynamics.github.io/aed-science/macrophytes---seagrass-and-macroalgae.html).

```{r, echo=FALSE, out.width="88%"}
knitr::include_graphics("./media/image117.jpeg")
```

:::{.figcap}
**Figure 16.5**. Overview of the aed_macrophyte model applied for Cockburn Sound seagrass biomass predictions.
:::
...
In general terms, we know that biomass and shoot (or leaf) density are related, though this is complicated when comparing between species, and also dependent on the specific "health" of any given meadow. For example, the same shoot density could be associated with a high or low biomass. Within this in mind we leverage the concept of the Interspecific Boundary Line (IBL) which sets a maximum efficiency of space occupation (Figure 16.6), and with the perpendicular distance to the IBL indicating the overall efficiency of space occupation of any particular data point (Vieira *et al.*, 2018).

```{r, echo=FALSE, out.width="90%"}
knitr::include_graphics("./media/image118.png")
```

:::{.figcap}
**Figure 16.6**. Seagrass aboveground biomass-shoot density relations worldwide. Biomass (B) and shoot density (D) of seagrasses, their interspecific boundary line (IBL) given by log10B = 4.569 − 0.438∙log10D, and stands' distances to the IBL (dgrass). Figure reprinted from Vieira *et al.* (2018).
:::

As observed in Figure 16.6, data from a broad range of species and across many locations show the trend following the perpendicular line, and the relative distance ($d_{grass}$) from the seagrass IBL. The perpendicular line represents the species specific biomass-shoot density relationship, and can be presented mathematically using a log function such as:
...
where ${MAC}_{\max}$ is the above-ground biomass under non-light limiting conditions and may be approximated from the IBL relationship outlined above, ${MAC}_{\min}$is the minimum biomass at a defined minimum depth, and $a\lbrack z\rbrack$ is a function based on the mean depth ($z$) at the location of interest. This function is optimised by fitting to data for *P. sinuosa* from Fraser and Kendrick (2017) as shown in Figure 16.8, giving, ${MAC}_{\min} =$ 10, ${MAC}_{\max} = 650$, and:

$$a\lbrack z\rbrack = 0.95 \times (z - 7.5)$$

:::{.figcap}
**Figure 16.8.** Biomass depth relationship based on two-years of data digitised from Collier *et al.* (2007) and Fraser and Kendrick (2017). The model is based on the optimal fit to the logistic equation above.
:::
...
where $f_{below}$ was computed from available data for *P. sinuosa* (Figure 16.9). Note that, whilst Eq above will set a fixed fraction of total biomass to be below-ground, this fraction will vary over time based on the dynamic rate of translocation used in the model.

:::{.figcap}
**Figure 16.9**. Above:below ground biomass relationship based on two-years of data digitised from Collier *et al.* (2007) and Fraser and Kendrick (2017).
:::
...
**Table 16.3**. Parameters used in the seagrass model ...
...
A simulation using CSIEM with the parameterisation of the model as described above has yielded accurate patterns in seagrass biomass seasonality and biomass distribution over depth. Figure 16.10 shows the validation site locations at three representative depths and the simulated seasonal dynamics of photon capture, above- and below-ground biomass, and fruit biomass, along with a comparison of modelled and observed above-ground biomass across depth ranges.

```{r, echo=FALSE, out.width="95%"}
knitr::include_graphics("./media/posidonia_validation.png")
```

:::{.figcap}
**Figure 16.10.** *Posidonia* biomass model validation. (a) Validation site locations at 1.5 m, 3.6 m and 7.1 m depth within Cockburn Sound. (b--e) Simulated seasonal time series of photons captured, above-ground biomass, below-ground biomass, and fruit biomass at three depths. (f) Comparison of modelled and observed above-ground biomass by depth range.
:::

Overall, when comparing the base model simulation with observed data aggregated over depth intervals, the model reasonably captures the gradient. There is an underprediction in the depth range from 2-6m, and slight overprediction at 8-10m.

## Filter feeder model

CSIEM v1.6+ can optionally include the effect of water filtration by benthic filter feeders. The filtration rate is spatially dependent on the specific features of each benthic layer (see Appendix B for an overview of the benthic layer parameterisation).

The model uses the `aed_bivalve` module in AED, and allows for benthic filter-feeder biomass (including C, N and P) to grow based on filtration of particulate matter. The biomasses used to initialise the model were based on Lemmens *et al.* (1996), who estimated epi-fauna and benthic fauna in several Cockburn Sound habitat areas (see above section).

An example output from the model is shown in Figure 16.11, which shows the filtration effect of filter-feeder biomass across the region. The modelled filter-feeder biomass is generally higher in the shallower areas of the Sound, which is consistent with the observed patterns from Lemmens *et al.* (1996), but overall the epi-benthic filter-feeder biomass had a more significant effect processing water outside of the Sound.

```{r, echo=FALSE, out.width="99%"}
knitr::include_graphics("./media/curtain_1panel_animation_2023-01-BIVALL.gif")
```

:::{.figcap}
**Figure 16.11.** Modelled filter-feeder (bivalve) filtered water fraction. The colour-scale represents water that has been filtered by a benthic macroinvertebrate within the past 7-days.
:::
...
The CSIEM platform includes dynamic benthic community modules that simulate the growth, productivity, and spatial distribution of seagrass and filter-feeder communities across Cockburn Sound and adjacent coastal waters. The seagrass model (`aed_macrophyte`) resolves above-ground leaf biomass, below-ground root/rhizome biomass, and seasonal fruiting for *Posidonia sinuosa*, driven by spectrally-resolved light capture, temperature-dependent photosynthesis and respiration, and above--below ground translocation. Model parameterisation draws on two decades of CSMC monitoring data (2003--2022), species-specific biomass--shoot density relationships derived from field surveys, and ecophysiological thresholds established through WAMSI Westport Marine Science Program experiments on light requirements and thermal tolerance. Validation against observed biomass across depth gradients shows that the model captures the broad depth-dependent patterns, with some underprediction in the 2--6 m range.

A benthic filter-feeder module (`aed_bivalve`) has also been incorporated, allowing the model to represent the filtration of particulate matter by epi-benthic invertebrates, initialised from historical biomass surveys (Lemmens *et al.*, 1996). The spatial distribution of benthic communities is defined through high-resolution habitat mapping supplied by WWMSP Theme 2 and O2M-Westport, with seagrass communities classified into four species groups for parameterisation within the model framework (Appendix B).

The current model configuration focuses on the water-quality controls on seagrass productivity---principally light availability and nutrient conditions. Additional stressors known to influence seagrass condition, including marine heatwave events (Chapter \@ref(heatwave); Strydom *et al.*, 2020), epiphyte loading (Cambridge *et al.*, 1986), and sediment sulfide intrusion (Fraser and Kendrick, 2017; Fraser *et al.*, 2023), are not yet fully represented and remain priorities for future model development. Nonetheless, the model provides a robust tool for predicting seagrass productivity responses to scenarios involving changes in water quality, such as those arising from dredging or nutrient enrichment.


<br>

```{block2, hintBenthicSummary, type='rmdsummary'}
- A dynamic seagrass model has been developed for *Posidonia sinuosa* within CSIEM, resolving above- and below-ground biomass dynamics driven by spectral light, temperature, and seasonal translocation processes
- Model parameterisation is informed by 20 years of CSMC monitoring data, field-derived biomass--depth relationships, and WWMSP2 experimental thresholds for light and temperature
- A benthic filter-feeder module simulates the filtration of particulate matter by epi-benthic invertebrates across the model domain
- High-resolution benthic habitat mapping (WWMSP2.1 / O2M-Westport) defines the spatial distribution of seagrass and substrate types used by the model
```
