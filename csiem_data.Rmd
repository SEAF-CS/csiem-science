...
<br>
<!--
```{r model-gnatt, echo=FALSE, message=FALSE, warning=FALSE}

library(tidyverse)

data <- read.csv("tables/modeldata.csv")

ggplot() +
  geom_segment(
    data,
    mapping = aes(
       x = as.Date(Start, format = "%d/%m/%Y"),
       xend = as.Date(End, format = "%d/%m/%Y"),
       y = reorder(project_num,-Order),
       yend = reorder(project_num,-order),
       colour = col
      ),
     size = 2, 
     lineend = "round"
    ) +
  geom_label(x = as.Date("02/07/2022", format = "%d/%m/%Y"), y = 1, label = "test")+
  geom_vline(xintercept = as.numeric(as.Date("01/01/2022", format = "%d/%m/%Y")), linetype=4)+
  geom_vline(xintercept = as.numeric(as.Date("01/01/2023", format = "%d/%m/%Y")), linetype=4)+
  scale_colour_manual(
  ...
  facet_wrap(~name, scales="free_y", nrow =9, ncol =1, shrink = FALSE, 
             strip.position = "left") +
  labs(y = "Project")+
  theme_classic() +
  theme(
    ...
    panel.grid.major.x = element_line(colour = "grey20", size = (0.1)),
    panel.grid.minor.x = element_line(linetype = "dashed",colour = "black", size = (0.5)),
    strip.text.y = element_text(hjust = 0.5, face = "bold", size = 8),
