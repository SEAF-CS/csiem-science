...
CSIEM seeks to integrate a diverse range of data from various origins and providers in order to allow the Cockburn Sound models to be informed by the best available data and science. However, the many types of data -- such as weather, hydrologic, water quality, and ecological data -- and the fact that the data spans many decades, creates a challenge to effectively organise the entire data-set in a robust, repeatable and transparent way. Several of the key data-sets are managed by different agencies who each use their own technical frameworks and standards for data organisation, and many other data-sets remain in an unorganised form, so integrating all the diverse datasets requires development of a common set of metadata standards and reliable data processing workflows. Providing this coordination to bring data-sets together is referred to as [data federation](https://www.sciencedirect.com/topics/computer-science/data-federation).

Data that is relevant for the management and modelling of Cockburn Sound comes from national data collection programs, state agency environmental monitoring programs, local companies and organisations, and various initiatives within the research community. These datasets are _all_ critical for understanding the Cockburn Sound ecosystem and form the foundation of the CSIEM system. Beyond simply organising data, the needs of the modelling require data in a consistent format (e.g. units, terminology, data structure, etc), to allow for effective model-data integration as is required during model set-up, validation and reporting phases. With this in mind, workflows have been developed to organise and store both field observational data and model data in a consistent way. The details of CSIEM data organisation are described in this chapter (Chapter 3), and the methodology for model-data integration is described in [Chapter 4](Model-Data Organisation).

All data management activities, processing pipelines, governance standards and curated data products are coordinated through the publicly accessible [`csiem-data`](https://github.com/SEAF-CS/csiem-data) GitHub repository, which serves as the central hub for the CSIEM Environmental Information Management framework. The full data ecosystem spans approximately 165 GB, with large datasets (the data lake and data warehouse) stored in external cloud storage, while GitHub tracks the processing code, governance files, variable and site keys, and data mapping resources (~216 MB).

This chapter summarises a synthesis of the various data sources into a standard _CSIEM Environmental Information Management_ framework that sets out standards and workflows. This framework is needed to:
...
(ref:data-pic1) Microsoft Azure Blob storage.

```{r data-pic1, echo=FALSE, fig.cap="(ref:data-pic1)", out.width="100%"}
...
(ref:data-pic2) WAMSI GIS portal.

```{r data-pic2, echo=FALSE, fig.cap="(ref:data-pic2)", out.width="100%"}
...
(ref:data-pic1a) Microsoft Sharepoint storage.

```{r data-pic1a, echo=FALSE, fig.cap="(ref:data-pic1a)", out.width="100%"}
knitr::include_graphics("images/cdm/blob.png")
```
...
(ref:data-pic3) Pawsey Acacia storage.

```{r data-pic3, echo=FALSE, fig.cap="(ref:data-pic3)", out.width="100%"}
...
The goal of the CSIEM Environmental Information Management framework, as presented herein, is to allow compatibility and inter-operability between critical data assets, and version control as is required for the development of a comprehensive and integrated modelling platform.
...
The relationship between the various initiatives, the CSIEM Environmental Information Management framework, and downstream model applications are outlined schematically in Figure \@ref(fig:data-pic4).

<br>

(ref:data-pic4) CSIEM Environmental Information Management overview (click to enlarge).

```{r data-pic4, echo=FALSE, fig.cap="(ref:data-pic4)", out.width="100%"}
knitr::include_graphics("images/cdm/Information_Management.png")
```

### Data Collation

The aim of the data collation step is to bring data together in a co-ordinated way. Data that is sourced and collated from various government agencies, researchers and industry groups is stored in a "data lake" (see Section 3.3.3) in its raw format. Each data provider is assigned a unique agency identifier, and datasets are also grouped based on the main programs or initiatives the collection was associated with. Raw data is stored in a rigid folder structure based on these two identifiers: <br> <br> `Agency/Program/ < ... data-sets ...> `

(ref:data-pic5) Data flow into the data lake. Raw data is allocated to folders based on the agency and program of origin, and version controlled within the csiem-data GitHub repository (right box).

```{r data-pic5, echo=FALSE, fig.cap="(ref:data-pic5)", out.width="100%"}
knitr::include_graphics("images/cdm/data_collation.png")
```

Data stored within the data lake is not formatted or processed in any way. Removal of duplicated and deprecated datasets is handled by the ETL (**"Extract, Transform, Load"**) pipelines outlined in Section 3.3.3. As such, strict data governance is required to maintain control and understanding of the contents of the data lake.

### Data Governance

Data Governance is the process of managing the availability and usability of data within the _CSIEM Environmental Information Management_ system. Governance can be loosely broken up into four key areas:
...
Each step has been designed to allow the users of the system to quickly understand the available data contained within the lake, the spatio-temporal extent that each data-set covers, as well as identify areas of missing or unavailable data (Figure \@ref(fig:data-pic7)). The final step is a version-controlled repository of the data, stored within the data-lake folder of the [`csiem-data`](https://github.com/SEAF-CS/csiem-data) GitHub repository.

(ref:data-pic7) Steps for governance of new and updated data-sets.

```{r data-pic7, echo=FALSE, fig.cap="(ref:data-pic7)", out.width="100%"}
...
In order to maintain the usability of the data lake, given its unstructured nature, it is important to ensure consistency when deciding where a particular data-set is to be stored. In the domain of the CSIEM Environmental Information Management system, all raw data is stored in the strict folder structure `agencycode/programcode`. If a particular agency program is unknown, a program code must be assigned during the cataloguing procedure. Multiple data-sets collected as part of a program can be organised freely.
...
All data entering the system is recorded in the `csiem_data_catalogue.xlsx` spreadsheet that can be found in the `data-governance` directory of the [`csiem-data`](https://github.com/SEAF-CS/csiem-data) GitHub repository. A full outline of the latest CSIEM data catalogue is included in **Appendix A: Data Catalogue**. A summary overview diagram is shown below in Figure \@ref(fig:data-flow).

(ref:data-flow) Data catalogue overview showing data sources organised by agency and programs (under development; click to enlarge).

```{r data-flow, echo=FALSE, fig.cap="(ref:data-flow)", out.width="100%"}
...
Independent GIS files are produced for each dataset added to the data lake. Not only does this allow for ease of mapping and visualisation, it serves as an important cross-check of the ingestion pipelines and any co-ordinate system conversions that have occurred within the pipeline.

Each dataset has a corresponding `csv` file saved within the `data-mapping` directory of [`csiem-data`](https://github.com/SEAF-CS/csiem-data), with the file name corresponding to the agencycode/programcode outlined above.
...
kbl(theSheet[,2:10], caption = "Example data-set GIS station location file, standardised for data-mapping.", align = "l",) %>%
...
The _CSIEM Environmental Information Management_ approach utilises a **Data Lake** --> **Data Warehouse** data management system, driven via custom batch **"ETL"** functions, triggered via GitHub Actions or able to be run manually. This system has been implemented due to both the vast array of raw data inputs required for integrated modelling, as well as the diverse data analytical demands required across different agencies and platforms.

#### GitHub

GitHub is a cloud-based git repository service, with enterprise-level version control and tracking. It has been chosen as the central repository for the data and the pipeline functions based upon its maturity as a platform, as well as its widespread adoption within industry and academia.

The [`csiem-data`](https://github.com/SEAF-CS/csiem-data) repository can be viewed and cloned from the [SEAF-CS GitHub organisation](https://github.com/SEAF-CS). The repository is structured around the following key directories:

- **`code/`**: Processing pipelines (MATLAB and Python) for data import, transformation, and analysis;
- **`data-governance/`**: Variable keys, site identifiers, and the master data catalogue;
- **`data-mapping/`**: Data transformation resources and standardised site location files;
- **`summary-images/`**: Public visualisations and example outputs.

```{r data-explorer-github, echo = FALSE, out.width='100%', class = "text-image", fig.align='center', fig.cap = "The csiem-data GitHub repository, which serves as the central hub for the CSIEM Environmental Information Management framework, housing processing pipelines, governance standards, variable keys, and data mapping resources."}

knitr::include_graphics("images/csiem/data_explorer_1.png")
...
However, because the data is primarily being stored in its raw format, governance becomes extremely important so that data assets are not lost in the lake.

#### Data Warehouse

The **Data Warehouse** is the store of boutique, customised data products produced through the batch **ETL (Extract, Transform and Load)** pipelines. Each folder within the `data-warehouse` directory contains processed data in different formats, based upon end-user requirements.

All data usage outside of the ETL pipelines should and must be carried out from products within the warehouse. This will ensure both the efficiency and repeatability of any script or data product produced downstream, as well as providing a constant data pathway for data validation.

#### Data Pipelines & ETL

Each dataset found within the data lake is extracted via the **ETL** pipeline utilising code found in the `code/import` directory. As each dataset is stored in its raw form, extraction and transformation more often than not requires a unique codebase. As such, a unified pipeline is not being developed at this stage, and each data source has its own codebase. To facilitate this, each codebase draws upon shared glossaries (such as the variable glossary discussed below) and conversion tables, to ensure standardisation.

(ref:data-pic9) ETL (Extract, Transform, Load) data workflow diagram.

```{r data-pic9, echo=FALSE, fig.cap="(ref:data-pic9)", out.width="100%"}
knitr::include_graphics("images/cdm/ETL.png")
```

#### GitHub Actions

One of the primary reasons for choosing GitHub to house the CSIEM Environmental Information Management system is GitHub Actions. GitHub Actions allows for the automation of workflows, using a variety of languages and technologies. Pipelines constructed in both Python and MATLAB can be automated and run in parallel via simple configurations, and can be processed on a wide variety of platforms and operating systems.
...
Model-derived data is used extensively throughout the research program, from providing direct input into other models (nesting) through to model validation. As such, model governance plays an equally important role as data governance. It is treated separately as model data is not stored directly in a single lake environment, but accessed and processed in a variety of ways depending on its size and storage location.

The Pawsey Acacia Storage is the primary home for model data produced by the different project teams. This storage space represents the long-term point-of-truth storage for these products. However, third-party produced models, as well as curated secondary products derived from primary models, are also stored on Acacia for the duration of the project. Model data such as BOM's BARRA-R output that is used directly as an input into a model, or used as a source for other data, is downloaded and stored, with any workflows produced directly referencing that Pawsey-stored data.

(ref:data-model) Model governance and storage.

```{r data-model, echo=FALSE, fig.cap="(ref:data-model)", out.width="100%"}
...
The details of data collection, processing, and categorisation have been described in Section 3.3 and Appendix A. This section introduces the data repository and management related to the data-model integration framework.
...
(ref:csiem-pic7) CSIEM Data online storage folder structure.

```{r csiem-pic7, echo=FALSE, fig.cap="(ref:csiem-pic7)", out.width="100%"}
...
All data that is uploaded to the CSIEM GitHub repository is logged in the `csiem-data` CSIEM Data Catalogue. The catalogue (found in the `data-governance` directory) is comprised of a main data sheet ("CSIEM Data Catalogue") as well as summary sheets for all processed data. Every raw data file is logged with the following information:
...
All of the ETL pipeline scripts for the CSIEM Environmental Information Management system are contained within the `code/` directory, which has three subfolders:
...
Scripting has been predominantly carried out utilising the [MATLAB](https://au.mathworks.com/?s_tid=gn_logo) programming language, however there are scripts within the `code` directory using both [R](https://www.r-project.org/about.html) and [Python](https://www.python.org/). Users are able to add scripts in their preferred language; the repository core scripts for data storage and model plotting are currently in MATLAB.
...
(ref:csiem-pic9) CSIEM work flow diagram for met data.

```{r csiem-pic9, echo=FALSE, fig.cap="(ref:csiem-pic9)", out.width="100%"}
...
The CSIEM Data Explorer is an interactive web-based dashboard that provides users with an accessible way to browse and inspect the datasets that have been processed into the `csiem-data` warehouse. The dashboard is hosted on the SEAF-CS Databricks platform and allows users to explore data availability across variables, data categories, and monitoring sites without needing to interact directly with the underlying code or file structures.

Users can select a data category (e.g. Water Quality, Meteorology, Hydrology) and a specific variable (e.g. O~2~ Saturation, Temperature, Salinity) to view an interactive map of all monitoring sites where that variable has been measured. Clicking on an individual site displays summary metadata -- including the data source agency and program, the number of data entries, the temporal coverage (first and last observation dates), basic statistics (mean, min, max), and depth information -- alongside a time-series plot of the observations. This allows researchers to quickly assess the spatial coverage and temporal extent of the observational record for any variable of interest, and to identify gaps or areas where data is sparse.

The Data Explorer draws directly from the curated data products in the `data-warehouse`, ensuring that all data shown has passed through the standardised ETL pipelines described in Section 3.5. As new datasets are ingested and processed into the warehouse, they become automatically available within the explorer.

```{r data-explorer-dashboard, echo = FALSE, out.width='100%', class = "text-image", fig.align='center', fig.cap = "The CSIEM Data Explorer dashboard showing available O2 Saturation monitoring sites across Perth coastal waters, with site-level metadata and a time-series preview for a selected WAMSI-WWMSP3 CTD station."}

knitr::include_graphics("images/csiem/data_explorer_2.png")
