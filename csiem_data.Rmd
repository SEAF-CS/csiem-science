...
CSIEM seeks to integrate a diverse range of data from various origins and providers in order to allow the COckburn Sound models developed to be infomred and integrated with the best available data and science. The many types of data, such as hydrologic, water quality, or ecological data, creates a challenge to effectively organise data in a robust, repeatable and transparanet way. The datasets are managed by different agencies who each use their own technical frameworks and standards for data organisation, so intgerating the diverse datasets requires development of a common set of metadata standards and reliable data processing workflows.

These datasets are critical for understanding the Cockburn Sound ecosystem and form the foundation of the CSIEM system. Beyond simply organising data, the needs of the modelling requires data in a consistent format (e.g. units, terminology, data structure, etc), to allow for effective model-data integration as is required during the model set up, validation and reporting phases. With this regard, workflows have been developed to organise and store both field observational data and model data in a consistent way. The details of CSIEM data organisation are described in this chpater (Chapter 3), and the methodology for model-data integration is described in [Chapter 4](Model-Data Organisation).

This Chapter summaries a synthesis of the various data sources into a standard, CSIEM Environmental Information Management framework that sets out standards and workflows. This will :
...
```{r data-pic1, echo = FALSE, out.width='70%', class = "text-image", fig.cap = "Microsoft Azure Blob Storage."}
...
```{r data-pic2, echo = FALSE, out.width='70%', class = "text-image", fig.cap = "WAMSI GIS Portal."}
...
### WAMSI Sharepoint {.unnumbered}

#### WAMSI Westport Reports and Data Sharepoint {.unnumbered}
...
### BMT Sharepoint {.unnumbered}

#### BMT Cockburn Sound Data SharePoint {.unnumbered}
