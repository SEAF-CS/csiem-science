...
The approach to the assessment of the model has been to consider both the broad-scale patterns - termed synoptic-scale assessment - follwoed by considering event-scale dynamics that allow a deeper look at key processes shaping water quality. 
...
An audit of the suitable data was undertaken to identify observed turbidity 'events' showing oxygen drawdown.  The main event periods with observed data available are:
...
In addition, an analysis of the Sentinel-2 imagery for Total Suspended Matter (TSM) is used to demonstrate the nature of spatial patterns in water turbidity, here assuming TSM is highly-correlated to turbidity. Note that the default TSM algorithm is used, without local calibration.

### Seasonal variability 

The Sentinel-2 TSM imagery provides useful context as the variability of turbidity over time. Noting that these images are biased to capture fair-weather (cloud free and glint free) conditions, they provide a picture of the background conditions. Figure 12.XX

![](./media/wq/TSM_mangles.png){width="90%"}

**Figure 12.XX.** Changes in water TSM within the CS_Mangles polygon. (a) ridgeplot showing TSM density in the selected polygon for each of 96 identified Sentinel-2 images. (b) selected image from March 2019 showing the observed TSM patterm within the harbour and location of the CS_Mangles polygon used to compute the ridgeplot distributions. Note that the ridgeplot distributions removed the "masked" regions indicated by grey to remove areas confounded by bottom reflectance issues.

Over the 6 years assessed
