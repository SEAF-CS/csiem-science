...
denitrification, or buried into deeper sediments. This cycling and movement of materials in surficial sediments, termed *early diagenesis*, is highly dynamic and regulated by factors such as $O_2$ concentration, organic matter availability
and physical processes at the sediment–water interface. In many coastal
...
of Cockburn Sound (Zhou *et al.*, 2024). Water quality has been monitored for decades and has improved following reductions in point-source nutrient inputs, such as wastewater treatment
...
clear isotopic signature, consistent with physical mixing or bioturbation.
...
nutrient state for the whole system, which might not be reversed. 
...
The differences that separate these environments are sediment type, light availability, organic matter loading and organic matter sources, which is associated with differences in $DIC$ and $O_2$ fluxes, and denitrification efficiency (Table \@ref(tab:sed-env)).
...
The area of each environment in Cockburn Sound was estimated using the habitat suitability index (HSI) from Chapter \@ref(habitat). The light component of HSI (scaled 0–1) was used to classify areas: values below 0.5 were assigned to the Deep-dark environment, consistent with the methodology of Zhai *et al.* Areas mapped as seagrass from surveys (Chapter \@ref(habitat)) were designated as the Seagrass environment, and remaining areas with light HSI above 0.5 were designated as the Benthic algae environment. The Benthic algae classification was based on light availability rather than direct surveys of benthic algal communities, and carries corresponding uncertainty, though it is consistent with the core data from Eyre *et al.* (2025).
...
library(kableExtra)
tbl_12_2 <- data.frame(
  Variable = c("Organic C", "Total N", "Total P", "Chl-a (mg m⁻²)",
               "Organic C", "Total N", "Total P", "Chl-a (mg m⁻²)"),
  Keesing = c("0.25", "0.05", "–", "50",
              "10 200", "2040", "–", "6"),
  Eyre = c("0.72", "0.12", "0.04", "83",
           "29 429", "5017", "1820", "9.9"),
  Zhou = c("3.0", "0.35", "–", "–",
           "122 400", "14 280", "–", "–"),
  check.names = FALSE
)
names(tbl_12_2) <- c("", "Keesing *et al.* (2011)", "Eyre *et al.* (2025)", "Zhou *et al.* (2024)")
kbl(tbl_12_2, caption = "Sediment nutrient concentrations and estimated mass storages in the top 2 cm of Cockburn Sound.", align = "l", escape = FALSE) %>%
  row_spec(0, background = "#14759e", bold = TRUE, color = "white") %>%
  pack_rows("Concentration (%)", 1, 4) %>%
  pack_rows("Mass storage (t)", 5, 8) %>%
...
```{r, echo=FALSE, out.width="95%"}
knitr::include_graphics("media/candi/candi_bc.png")
```
...
The model domain extended to 30 cm depth, discretised into 400 equally-sized vertical layers (0.75 mm each); according to Zhou *et al.* (2024), 30 cm depth corresponds to a sediment deposition year of approximately 1992. The model was run with daily time steps. Bioturbation was set to 50 cm$^2$ y\(^{-1}\) at the sediment–water interface, decaying exponentially to zero at 8 cm depth. A 5-year spin-up with constant boundary fluxes and concentrations was used to bring the system to equilibrium, after which the time-varying boundary conditions from the water column model were applied. Results plots below focus on the upper 15–20 cm of the 30 cm domain.

```{r, echo=FALSE}
library(kableExtra)
tbl_12_3 <- data.frame(
  Variable = c("$POML$", "$POMR$", "$DIC$", "$O_2$", "$NH_4^+$", "$NO_3^-$",
               "$NO_2^-$", "$N_2O$", "$N_2$", "Salinity", "$SO_4^{2-}$",
               "$PO_4^{3-}$", "$MnO_2A$", "$Fe(OH)_3A$"),
  `Spinup boundary` = c(
    "8470 mmol m⁻² y⁻¹", "2000 mmol m⁻² y⁻¹",
    "2 mmol L⁻¹", "0.200 mmol L⁻¹",
    "0.022 mmol L⁻¹", "0.056 mmol L⁻¹",
    "0.024 mmol L⁻¹", "0 mmol L⁻¹",
    "0.4 mmol L⁻¹", "35 PSU",
    "31 mmol L⁻¹", "0.0002 mmol L⁻¹",
    "400 mmol m⁻² y⁻¹", "750 mmol m⁻² y⁻¹"),
  `Dynamic boundary average` = c(
    "8471 mmol m⁻² y⁻¹", "2118 mmol m⁻² y⁻¹",
    "–", "0.206 mmol L⁻¹",
    "0.0017 mmol L⁻¹", "0.006 mmol L⁻¹",
    "0.002 mmol L⁻¹", "–",
    "–", "36 PSU",
    "32 mmol L⁻¹", "0.0002 mmol L⁻¹",
    "–", "–"),
  `Target or source` = c(
    "Produce $DIC$ 8600 mmol m⁻² y⁻¹ efflux",
    "Produce 5000 mmol L⁻¹ solids (1–3% TOC)",
    "Van Cappellen and Wang (1996)",
    "Uptake of ~9700 mmol m⁻² y⁻¹",
    "Efflux 50 mmol m⁻² y⁻¹",
    "Uptake 150 mmol m⁻² y⁻¹",
    "Comparable to $N_2$ and $NO_3^-$",
    "Comparable to $N_2$ and $NO_3^-$",
    "Efflux 350 mmol m⁻² y⁻¹",
    "",
    "Small consumption of $SO_4^{2-}$",
    "",
    "Van Cappellen and Wang (1996)",
    "Van Cappellen and Wang (1996)"),
  check.names = FALSE
)
kbl(tbl_12_3, caption = "Summary of key variables and their boundary conditions.", align = "l", escape = FALSE) %>%
  row_spec(0, background = "#14759e", bold = TRUE, color = "white") %>%
  kable_styling(full_width = F, font_size = 10)
```

#### Organic matter setup {.unnumbered}
...
#### Denitrification model {.unnumbered}
...
##### N redox rate constants {.unnumbered}
...
:::{.figcap}
**Figure 12.7-ii.** N redox model with the rate constants or partitioning constants drawn over the pathway arrows.
:::
...
:::{.figcap}
**Figure 12.8.** Deep aerobic conceptual model.
:::

#### C:N ratio {.unnumbered}
...
#### Deep-dark sediment environment {.unnumbered}
...
- $N_2$ flux should have been around 200 mmol m\(^{-2}\) y\(^{-1}\), according to the measured data. There should have been a small amount of denitrification at the depth where $O_2$ concentration declined, and where  $O_2$ did not inhibit denitrification reactions.
...
#### Benthic algae sediment environment {.unnumbered}
...
Flux of $DIC$ and $O_2$ at the sediment-water interface should have been very small or close to zero, as measured in the cores. The flux of $N_2$ was around 200 to 350 mmol m\(^{-2}\) y\(^{-1}\) in cores 7 and 8.
...
- 100 mg Chl-a m\(^{-2}\) / 12 (convert chlorophyll per area)
- 8 mmol Chl-a m\(^{-2}\) / 0.002 m (over the top 2 mm)
- 4000 mmol Chl-a m\(^{-3}\) / 1000 L m\(^{-3}\)
- 4 mmol Chl-a L\(^{-1}\) × 40 C Chl-a\(^{-1}\) (assume 40 mol biomass C per Chl-a)
- 150 mmol C L\(^{-1}\)
...
#### Seagrass sediment environment {.unnumbered}
...
- Roots assimilated $DIC$ and exuded $O_2$ in the sediment, at a 1:1 molar ratio (Figure 12.13)
...
- Seagrass detritus reaction rate constant was 1 y\(^{-1}\) (the $POML$  reaction rate was 2 y\(^{-1}\))
...
- From the $MPB$ setup, $MPB$ NPP was 13 000 mmol m\(^{-2}\) y\(^{-1}\)
  ...
  therefore roots $O_2$ = 42 000 - 13 000
...
#### Seasonal simulation setup {.unnumbered}
...
#### High organic matter period setup {.unnumbered}
...
#### Groundwater observations {.unnumbered}

The equilibrium simulation was extended with groundwater inflow into the deepest layer of sediment. The motivation for including this environment followed from the N flux budgets of Greenwood *et al.* (2016), who estimated groundwater TN inputs of 450 to 920 t y\(^{-1}\) depending on rainfall variability (after Donn *et al.*, 2015), compared with earlier estimates of 300 to 2100 t N y\(^{-1}\) (CSMC, 2001) that combined groundwater, atmospheric and catchment sources. Although internal N recycling in Cockburn Sound is on the order of thousands of t y\(^{-1}\), groundwater constitutes the dominant external TN source, given that atmospheric and northern sea boundary inputs are approximately 10 and 51 t y\(^{-1}\), respectively (Greenwood *et al.*, 2016). Combined burial and denitrification were proposed to balance the groundwater input, though uncertainty remains. Furthermore, the Greenwood *et al.* (2016) budget assumed that sediment TN release occurred as $NO_3^-$ rather than $NH_4^+$ or other $DIN$ species. A key objective of the CANDI-AED simulations was therefore to estimate both the magnitude and speciation of sediment $DIN$ fluxes, including the contribution from groundwater.
...
tbl_12_4 <- data.frame(
  Variable = c(
    "CSIEM zone", "Area (km²)", "Avg pf (y⁻¹)",
    "Avg flux (mmol m⁻² y⁻¹)", "Avg load (t y⁻¹)",
    "SO₄²⁻", "Total N", "NH₄⁺", "NO₃⁻ + NO₂⁻", "PO₄³⁻", "DIC"
  ),
  Beach_high = c("Zone 15", "0.61", "616", "3758", "32",
                  "22.3", "0.080", "0.031", "0.049", "7.88e-5", "4.0"),
  Beach_low  = c("Zone 17", "0.42", "435", "1827", "11",
                  "19.4", "0.084", "0.039", "0.045", "1.05e-4", "3.9"),
  Shelf_high = c("Zone 8", "1.31", "12", "157", "3",
                  "24.5", "0.69", "0.69", "0.00", "9.42e-5", "2.7"),
  Shelf_low  = c("Zone 3", "0.48", "73", "350", "2",
                  "24.3", "0.04", "0.04", "0.00", "8.60e-5", "3.9"),
  check.names = FALSE
)
names(tbl_12_4) <- c("", "Beach high", "Beach low", "Shelf high", "Shelf low")
kbl(tbl_12_4,
    caption = "Summary of groundwater zone properties and porewater concentrations (mmol L<sup>-1</sup>) for the four groundwater environments.",
    align = "l", escape = FALSE) %>%
  row_spec(0, background = "#14759e", bold = TRUE, color = "white") %>%
  pack_rows("Zone properties", 1, 5) %>%
  pack_rows("Porewater concentrations (mmol L⁻¹)", 6, 11) %>%
  kable_styling(full_width = F, font_size = 10)
```
...
#### Groundwater processes and assumptions {.unnumbered}
...
#### Deep-dark environment steady results {.unnumbered}
...
#### Benthic algae environment steady results {.unnumbered}
...
#### Seagrass environment steady results {.unnumbered}
...
#### Flux and mass N budgets {.unnumbered}
...
library(kableExtra)
tbl_12_6 <- data.frame(
  Environment = c("Deep-dark", "Benthic algae", "Seagrass", "**Sum**"),
  `Total net N` = c(-373, -341, -320, -1034),
  `POML + Seagrass detritus` = c(1819, 664, 604, 3087),
  `NH4+` = c(-104, -233, -94, -431),
  `NO3-` = c(28, 10, 0, 38),
  `NO2-` = c(-208, -96, -203, -506),
  `N2O` = c(-86, -23, -23, -132),
  `N2` = c(-1087, -664, -261, -2012),
  `Total N (store)` = c(2215, 888, 313, 3417),
  `Organic N (store)` = c(2195, 881, 310, 3387),
  check.names = FALSE
)
kbl(tbl_12_6, caption = "N budget fluxes (t y<sup>-1</sup>) and N stores in the top 2 cm of sediment (t) for each environment. Negative flux indicates transport from the sediment to the water.", align = "l", escape = FALSE) %>%
  row_spec(0, background = "#14759e", bold = TRUE, color = "white") %>%
...
#### Seasonal variability results {.unnumbered}
...
tbl_12_7 <- data.frame(
  Environment = c("Deep-dark", "Benthic algae", "Seagrass", "**Sum**"),
  `Total net N` = c(-1508, -999, -577, -3084),
  `POML + Seagrass detritus` = c(1517, 1021, 587, 3124),
  `NH4+` = c(-51, -203, -84, -338),
  `NO3-` = c(28, 10, -1, 37),
  `NO2-` = c(-249, -108, -203, -559),
  `N2` = c(-1148, -673, -267, -2087),
  `N2O` = c(-90, -24, -23, -137),
  check.names = FALSE
)
kbl(tbl_12_7, caption = "N budget fluxes in t y<sup>-1</sup> from each sediment environment and the total, during the seasonally fluctuating simulation. Negative indicates flux from the sediment to the water.", align = "l", escape = FALSE) %>%
  row_spec(0, background = "#14759e", bold = TRUE, color = "white") %>%
  kable_styling(full_width = F, font_size = 10)
```

#### Sediment response to low $O_2$ event {.unnumbered}
...
#### Sediment response to high organic matter loading {.unnumbered}
...
:::{.figcap}
**Figure 12.32-iv.** Summary figure of $N_2$ under organic matter increase. Left to right – Deep-dark, Benthic algae communities and Seagrass environments. Top – flux over time. Bottom – plot of concentration, depth and time, with the bottom water shown above the sediment-water interface.
:::

### Sensitivity of sediment processes to N-rich groundwater loading
...
```{r, echo=FALSE, out.width="95%"}
knitr::include_graphics("media/candi/gw_flux.png")
```
...
```{r, echo=FALSE}
tbl_12_8 <- data.frame(
  Environment = c("Deep-dark", "Beach high", "Beach low", "Shelf high", "Shelf low"),
  `NH4+` = c(-93, -163, -154, -105, -98),
  `NO3-` = c(25, 3, 14, 25, 25),
  `NO2-` = c(-188, -166, -169, -187, -186),
  `NOx-` = c(-163, -163, -155, -162, -161),
  `N2` = c(-483, -4500, -2940, -527, -646),
  `O2` = c(8160, 8450, 8400, 8160, 8160),
  `PO43-` = c(-81, -82, -82, -81, -81),
  check.names = FALSE
)
kbl(tbl_12_8, caption = "Average fluxes (mmol N m<sup>-2</sup> y<sup>-1</sup>) for key variables for the four groundwater environments compared to the baseline Deep-dark flux.", align = "l", escape = FALSE) %>%
  row_spec(0, background = "#14759e", bold = TRUE, color = "white") %>%
  kable_styling(full_width = F, font_size = 10)
```
...
```{r, echo=FALSE, out.width="95%"}
knitr::include_graphics("media/candi/gw_N.png")
```
