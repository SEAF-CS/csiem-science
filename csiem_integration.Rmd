...
- csiem-science: a repository for documenting the CSIEM workflows, model-data structure and integration, and results;
- csiem-data: platform for storing and sharing environmental data for this project;
- csiem-model: platform for storing and sharing model files and configurations;
- csiem-marvl: a collection of scripts for visualising and assessing the model performance.
...
In particular, the publicly available GitHub repository called [`csiem-marvl`](https://github.com/AquaticEcoDynamics/csiem-marvl) is used and contains a wide variety of matlab scripts and functions that are frequently used to process and visualise model output. `AEDmarvl_plot_timeseries` and `AEDmarvl_plot_transect` are the main functions that are frequently used. Tables 7.3 and 7.6 in Appendix B are examples of output generated via the `AEDmarvl_plot_timeseries` function found in the `csiem-marvl` repository. This plotting function uses data and gis files stored in the csiem-marvl repository, in addition to model output to create timeseries plots of the model (averaged within a polygon region) compared against field data. The plotting function will also automatically calculate a range of error statistics based on model output and field measured data.
