...
The condition of seagrass communities in Cockburn Sound, Warnbro Sound and Marmion at various sites were monitored annually between 2003 -- 2022 by Cockburn Sound Management Council (Figure 14.1). Sampling was mostly conducted during summer months but has varied from year to year. Survey records generally include shoot count, lower depth limit, seagrass height, dominant epiphyte type and cover (Table 14.1), but also varied over time. Monitoring primarily focused on the historically dominant species *Posidonia sinuosa*, though in some years, presence/absence or shoot count data are also available for other species such as *Posidonia australis*, *Posidonia coriacea*, *Amphibolis griffithii*, and *Amphibolis antarctica*. Average shoot count of *P. sinuosa* between 2003 - 2022 at all monitoring sites within the model domain are summarized in Figure 14.2 and 14.3. Additionally, several case studies that recorded *P. sinuosa* biomass in CS are particularly relevant to the dynamic seagrass model and are listed in Table 14.1.
...
:::{.figcap}
**Figure 14.1**. Location of CSMC long-term seagrass monitoring sites within the model domain (left) and within Cockburn Sound (right).
:::

**Table 14.1.** Summary of historical seagrass monitoring datasets in Cockburn Sound relevant to model development.
...
:::{.figcap}
**Figure 14.2**. Average *Posidonia sinuosa* shoot count per 20 cm^2^ between 2003 -- 2022 at CSMC monitoring sites (showing sites within model domain only).
:::
...
:::{.figcap}
**Figure 14.3**. Average *Posidonia sinuosa* shoot count per 20 cm^2^ between 2003 -- 2022 at CSMC monitoring sites Garden Island and Warnbro Sound at multiple depths.
:::
...
Due to the high resolution of the supplied benthic habitat maps and technical constraints of TUFLOW-FV, polygon boundaries underwent a resolution reduction process prior to being used in model simulation (Figure 14.4). Additionally, the original seagrass community groups provided by WWMSP2.1 were classified into four groups by their dominant species (Table 14.2) to assist with alignment of group parameterisation within CSIEM. Both benthic map versions (WWMSP2.1 and WWMSP2.1-O2M-Westport merged) are compatible with CSIEM and can be selected during configuration. 


**Table 14.2**. Seagrass groups in WWMSP2.1 benthic map and CSIEM set up.
...
:::{.figcap}
**Figure 14.4-i**. Substrate type and benthic habitat within the model domain based on WWMSP2.1 mapping, including distribution of seagrass communities within the Cockburn Sound and Owen Anchorage (CSOA). Click to enlarge.
:::
...
:::{.figcap}
**Figure 14.4-ii**. Substrate type and benthic habitat within the model domain based on merged WWMSP2.1-O2M-Westport mapping, including distribution of seagrass communities within the Cockburn Sound and Owen Anchorage (CSOA). Click to enlarge.
:::
...
:::{.figcap}
**Figure 14.13**. Overview of the aed_macrophyte model applied for Cockburn Sound seagrass biomass predictions.
:::
...
In general terms, we know that biomass and shoot (or leaf) density are related, though this is complicated when comparing between species, and also dependent on the specific "health" of any given meadow. For example, the same shoot density could be associated with a high or low biomass. Within this in mind we leverage the concept of the Interspecific Boundary Line (IBL) which sets a maximum efficiency of space occupation (Figure 14.14), and with the perpendicular distance to the IBL indicating the overall efficiency of space occupation of any particular data point (Vieira et al., 2018).
...
:::{.figcap}
**Figure 14.14**. Seagrass aboveground biomass-shoot density relations worldwide. Biomass (B) and shoot density (D) of seagrasses, their interspecific boundary line (IBL) given by log10B = 4.569 − 0.438∙log10D, and stands' distances to the IBL (dgrass). Figure reprinted from Vieira et al. (2018).
:::

As observed in Figure 14.14, data from a broad range of species and across many locations show the trend following the perpendicular line, and the relative distance ($d_{grass}$) from the seagrass IBL. The perpendicular line represents the species specific biomass-shoot density relationship, and can be presented mathematically using a log function such as:
...
In Cockburn Sound, biomass-shoot density relation for the dominant species *P. sinuosa* has been approximated using survey data from 1993, 2003 and 2015 where both shoot density and biomass records were available (Figure 14.15).
...
:::{.figcap}
**Figure 14.16.** Biomass depth relationship based on two-years of data digitised from Collier et al. (2007) and Fraser and Kendrick (2017). The model is based on optimal fit to Eq XX.
:::
...
where $f_{below}$ was computed from available data for *P. sinuosa* (Figure 14.17). Note that, whilst Eq above will set a fixed fraction of total biomass to be below-ground, this fraction will vary over time based on the dynamic rate of translocation used in the model.

:::{.figcap}
**Figure 14.17**. Above:below ground biomass relationship based on two-years of data digitised from Collier et al. (2007) and Fraser and Kendrick (2017).
:::
...
**Table 14.4**. Parameters used in the seagrass model ...
...
:::{.figcap}
**Figure 14.18.** Modelled *Posidonia* group biomass.
:::
...
:::{.figcap}
**Figure 14.19.** Modelled *Posidonia* group biomass seasonally varying, at multiple depths.
:::
...
:::{.figcap}
**Figure 14.20.** Modelled *Posidonia* group biomass compared with the average observed biomass from sampling for the year 2022.
:::
