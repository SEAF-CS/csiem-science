...
functional types ("PFT's") that are typically defined based on specific
...
community in CSIEM has been configured into four PFTs, according to literature review of their dominant traits and the
...
denoted $grn$, $crypt$, $diatom$ and $dino$. The parameters used
...
phytoplankton along a h

Direct measurements for other group-specific physiological rates and
parameters are not available for this coastal region, and so the current
...
from the estuaries and, advection of material through the system, and vertical
fluxes due to particle resuspension and deposition.
CSIEM includes 2 sediment particles sizes ($SS_s$) and is
...
in the coastal environment covered by the CSIEM domain.
...
the WWM wave model is included within CSIEM. The wave model predicts the spatial
...
resuspension is critical to predict the turbidity of the
water which controls the underlying light climate that shapes the dynamics
...
was categorized to XX regions in order to categorise different sediment
properties between the shallow and deep areas and north to south (Figure
 ).

#### Turbidity

Computing turbidity from the concentration of particulates is included in the model able to be compared to routinely measured turbidity data. The relation for simulation of turbidity is expressed as:
...
### AED model parameterisation : benthic zones

A detailed summary of zone specific parameters is outlined in [Appendix B](benthicapp).
